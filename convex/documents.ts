...
export const getSidebar = query({
    ...
    handler: async (ctx, args) => {
        ...
        if(!identity) {
            throw new Error('Unautheticated user')
        }

        const userId = identity.subject

        const documents = ctx.db.query('documents').withIndex('by_user_parent', (q) => q.eq('userId', userId).eq('parentDocument', args.parentDocument)).filter((q) => q.eq(q.field('isArchived'), false)).order('desc').collect()
